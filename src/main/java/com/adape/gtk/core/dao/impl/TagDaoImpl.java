...
import com.adape.gtk.core.dao.entity.repository.TagRepository;
...
public class TagDaoImpl implements TagDao{
	...
	@Override
	public Tag create(Tag tag) throws CustomException {
		Tag newTag = null;
		try {
	...
	@Override
	public Tag edit(Tag tag) throws CustomException {
		Tag newTag = null;
		try {	
			newTag = tagRepository.save(tag);
		} catch (ConstraintViolationException cve) {
	...
	@Override
	public List<Integer> delete(List<Tag> id) throws CustomException {
	...
	public boolean existsById(Integer id) {
		if (tagRepository.existsById(id)) {
			log.info(String.format(Constants.ENTITY_EXIST, "Tag"));
			return true;
		} else {
			log.info(String.format(Constants.ENTITY_NOT_EXIST, "Tag"));
			return false;
		}
	}
	...
	public Tag get(Integer id) {
		...
		if (optTag.isEmpty()) {
			log.info(String.format(Constants.ENTITY_GET_NOT_FOUND, "Tag", "id: " + id));
			return null;
		} else {
			log.info(String.format(Constants.ENTITY_GET_SUCCESSFULLY, "Tag", "id: " + id));	
			return optTag.get();
		}
	}

	@Override
	public Response<Tag> get(Filter filter) throws CustomException{
		...
		try {
			...
			CriteriaQuery<Tag> selectCount = query.select(root.get("id")).distinct(true).where(predicates.toArray(new Predicate[predicates.size()]));
			Long size = Long.valueOf(entityManager.createQuery(selectCount).getResultList().size());
			
			CriteriaQuery<Tag> select = query.select(root).distinct(true).where(predicates.toArray(new Predicate[predicates.size()]));
			TypedQuery<Tag> typedQuery = entityManager.createQuery(select)
					.setFirstResult(pageNo*pageSize)
					.setMaxResults(pageSize);

			Long pages = (long) Math.ceil(size.doubleValue()/pageSize);
			return new Response<Tag>(size,typedQuery.getResultList(), pages);
		} catch (Exception e) {
			throw new CustomException(500, e);
		}
	}
