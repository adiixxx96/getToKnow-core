...
public class CommentIntServiceImpl implements CommentIntService{
	...
	private String host;
	@Value("${CommentCreate.url:#{'/comment/createComment'}}")
	private String urlCreate;
	@Value("${CommentEdit.url:#{'/comment/editComment'}}/")
	private String urlUpdate;
	@Value("${CommentDelete.url:#{'/comment/deleteComment'}}")
	private String urlDelete;
	@Value("${CommentGet.url:#{'/comment/getComment'}}/")
	private String urlGet;
	@Value("${CommentGetFilter.url:#{'/comment/getComments'}}")
	private String urlGetFilter;
	

	@Override
	public ResponseMessage create(CommentDTO Dto, int userId) {
	    ...
	    try {
	      ResponseEntity<CommentDTO> response = clientRest.exchange(url, HttpMethod.POST, new HttpEntity<CommentDTO>(Dto, headers), CommentDTO.class);
	...
	@Override
	public ResponseMessage edit(CommentDTO Dto, int userId) {
	    ResponseMessage responseEntity = new ResponseMessage();
	    ...
	    try {
	      ResponseEntity<CommentDTO> response = clientRest.exchange(url, HttpMethod.PUT, new HttpEntity<CommentDTO>(Dto, headers), CommentDTO.class);
	      responseEntity.setStatus(HttpStatus.OK);
	      responseEntity.setMessage(response.getBody());
	      log.info("ClientRestResponse: {}", responseEntity);
	    } catch (RestClientResponseException ex) {
	...
	public ResponseMessage get(Integer id, int userId) {
	    ...
	    log.trace(CALLING, url);
	    try {
	      ResponseEntity<CommentDTO> response = clientRest.exchange(url, HttpMethod.GET, null, CommentDTO.class);
	      responseEntity.setStatus(HttpStatus.OK);
	      responseEntity.setMessage(response.getBody());
	      log.info("ClientRestResponse: {}", responseEntity);
	    } catch (RestClientResponseException ex) {
	...
	public ResponseMessage get(Filter filter, int userId) {
		...
		ResponseMessage responseEntity = new ResponseMessage();
	      ...
	      log.trace(CALLING, url);
	      try {
	        ResponseEntity<Response<CommentDTO>> response = clientRest.exchange(url, HttpMethod.POST, new HttpEntity<Filter>(filter),
					new ParameterizedTypeReference<Response<CommentDTO>>() {});
