...
public class LiteralIntServiceImpl implements LiteralIntService{
	...
	private String host;
	@Value("${LiteralCreate.url:#{'/literal/createLiteral'}}")
	private String urlCreate;
	@Value("${LiteralEdit.url:#{'/literal/editLiteral'}}/")
	private String urlUpdate;
	@Value("${LiteralDelete.url:#{'/literal/deleteLiteral'}}")
	private String urlDelete;
	@Value("${LiteralGet.url:#{'/literal/getLiteral'}}/")
	private String urlGet;
	@Value("${LiteralGetFilter.url:#{'/literal/getLiterals'}}")
	private String urlGetFilter;
	

	@Override
	public ResponseMessage create(LiteralDTO Dto, int userId) {
	    ...
	    try {
	      ResponseEntity<LiteralDTO> response = clientRest.exchange(url, HttpMethod.POST, new HttpEntity<LiteralDTO>(Dto, headers), LiteralDTO.class);
	...
	@Override
	public ResponseMessage edit(LiteralDTO Dto, int userId) {
	    ResponseMessage responseEntity = new ResponseMessage();
	    ...
	    try {
	      ResponseEntity<LiteralDTO> response = clientRest.exchange(url, HttpMethod.PUT, new HttpEntity<LiteralDTO>(Dto, headers), LiteralDTO.class);
	      responseEntity.setStatus(HttpStatus.OK);
	      responseEntity.setMessage(response.getBody());
	      log.info("ClientRestResponse: {}", responseEntity);
	    } catch (RestClientResponseException ex) {
	...
	public ResponseMessage get(Integer id, int userId) {
	    ...
	    log.trace(CALLING, url);
	    try {
	      ResponseEntity<LiteralDTO> response = clientRest.exchange(url, HttpMethod.GET, null, LiteralDTO.class);
	      responseEntity.setStatus(HttpStatus.OK);
	      responseEntity.setMessage(response.getBody());
	      log.info("ClientRestResponse: {}", responseEntity);
	    } catch (RestClientResponseException ex) {
	...
	public ResponseMessage get(Filter filter, int userId) {
		...
		ResponseMessage responseEntity = new ResponseMessage();
	      ...
	      log.trace(CALLING, url);
	      try {
	        ResponseEntity<Response<LiteralDTO>> response = clientRest.exchange(url, HttpMethod.POST, new HttpEntity<Filter>(filter),
					new ParameterizedTypeReference<Response<LiteralDTO>>() {});
